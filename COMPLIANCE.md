...
Checked against, not invented in isolation: Pega Robot Studio's persistent, technology-neutral `Control` resolved by match rules to a runtime `Target`; Blue Prism's Application Model fed by interchangeable Win32/UIA/Java/HTML/Vision "spy" technologies; Microsoft UI Automation's split between control *type* and control *pattern*; UiPath's Unified Target layering selectors, anchors, and computer vision behind one logical target. Four independently-built platforms converging on the same shape — a technology-neutral logical control resolved by ranked, surface-specific strategies — is a strong signal `targetRegistry`'s design is the right one.
...
Rung 5's `{rowHeader, columnHeader}` shape and `framePath` both come from a specific, real prior experience worth naming rather than treating as one RPA vendor among four: automating CTRM (commodity trade & risk management) surfaces with source-code access to the target application, where the natural model was a strict object hierarchy — `Application → View → Pane → Grid → Row → Cell → Control` — and every control's supported actions were mapped once against that hierarchy, not once per automated workflow. `structural_semantic` and `framePath` are that hierarchy's `Grid/Row/Cell` and `View/Pane` layers, reconstructed here without the advantage that made the original version easy: no source access, so the hierarchy has to be re-derived from whatever the surface exposes at runtime (DOM, accessibility tree, screenshot) instead of read from a schema the target's own authors handed over. `Target.candidates[]` — an ordered, typed, ranked set of ways to re-derive one logical control — is this project's answer to that harder version of the same problem.
...
The per-capability model above has a real limit — `targetRegistry` lives *inside* each capability, so every capability on the same app re-declares its own copy of every control it shares with siblings. A forward-design proposal addressing that (a shared, vendor/app-scoped component library, plus making discovery's own LLM provider pluggable rather than Anthropic-only) is written up separately in **[`FORWARD_DESIGN.md`](FORWARD_DESIGN.md)** rather than here, since it's additive design on top of what's built, not a description of it.
